...
| Release | Added |
|---------|-------|
| v4.4.1 | Everything in v4.3.5, plus: dynamic DNS routes a mixed `myip` list to A and AAAA records; `/records/bulk` is matched before the single-record lookup; the PowerDNS metrics URL is derived from `pdns_api.url`, including path prefixes; `PA_DNS_TTL_REVERSE`, `PA_DNS_DEFAULT_ZONE_TEMPLATE`, `PA_DNS_ZONE_OWNERSHIP_MODE`, `PA_DNS_SYNC_ZONE_OWNER_TO_ACCOUNT`, `PA_FAVICON_PATH`, `PA_LOGO_PATH` and `PA_PDNS_API_TIMEOUT` container variables for the 4.4.0 settings |

> **Warning:** v4.4.1 changed the same five confirmation pages as v4.2.6 (POST forms with a
> CSRF token), and the record lock in `edit.html` now covers LUA records. Re-sync forks of
> `delete_perm_templ.html`, `delete_zone_templ.html`, `delete_zone_templ_record.html`,
> `delete_supermaster.html`, `dnssec_edit_key.html` and `edit.html`.
