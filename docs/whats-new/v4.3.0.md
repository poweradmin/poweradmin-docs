...
| v4.3.5 | Everything in v4.2.6, plus: LUA records restricted for client-level editors in the web form, the API and zone templates, with existing ones shown locked like SOA and NS; one canonical zone id in API backend mode so rows sharing an identifier no longer collapse; container config generation escapes secrets and emits valid PHP for boolean values |
...
> **Warning:** v4.3.5 changed the same five confirmation pages as v4.2.6 (POST forms with a
