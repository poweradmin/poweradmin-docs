...
> **Warning:** Not yet released. 4.5.0 is complete on the `master` branch and awaiting its tag.
> The latest release is v4.4.1, and 4.3.x remains the stable line recommended for production.
> This page describes what the release will contain.
...
|---|---|---|
| NAPTR service field checked properly | The service field is validated as parts joined by plus signs, each starting with a letter, so a trailing plus (`"E2U+"`) or an empty part (`"aaa++diameter"`) is refused instead of accepted | [NAPTR Records](../user-guide/naptr-records.md) |
| Login page no longer loops | A session holding a user id the dashboard treats as logged out made `/` and `/login` redirect to each other until the browser gave up, with no way to reach the form. Both now decide the same way | |
...
| Health and ping endpoints | `/ping` answers while the database is down, `/api/health` reports whether the database and PowerDNS API are reachable. Both are unauthenticated and off by default | [Health Checks](../operations/health-checks.md) |
| Headless mode | `interface.web_enabled = false` serves the API and nothing else, for installations driven entirely by Terraform, ExternalDNS or scripts | [Headless Quickstart](../getting-started/headless-quickstart.md) |
| Fewer connections on public paths | Static assets, the removed v1 API and the monitoring probes no longer open a database connection per request | [Health Checks](../operations/health-checks.md) |
...
| Matching PTR written to the right zone | With the API backend, a matching PTR created alongside an A or AAAA record could be written into an unrelated reverse zone while the page reported success. The reverse-zone lookup now returns the same canonical id the rest of the backend uses | [PowerDNS API](../configuration/powerdns-api.md) |
| OIDC verified-email linking | `email_verified` is read from the ID token as well as the userinfo response, and `oidc.require_verified_email` can demand the claim before an address may match an existing account | [OIDC Authentication](../configuration/oidc.md) |
...
| Longer template record types | `NSEC3PARAM`, `OPENPGPKEY` and other types over six characters can be stored in zone templates | [DNS Templates](../user-guide/dns-templates.md) |
| IDN zone handling | Imported internationalised names are converted to punycode, partial searches match decoded names, and IDN zones are indexed under their real initial rather than all under "x" | [Zone Import and Export](../configuration/zone-import-export.md) |
| Log level typos no longer silence logging | An unrecognised `logging.level` falls back to `info` and is reported, instead of suppressing everything below `emergency` | [Basic Logging Configuration](../configuration/logging.md) |
