...
| Internals reorganised | The code under `lib/` now follows its layers strictly: domain services are grouped by concern (`Domain/Service/Auth`, `Zone`, `Template`, `Dns`, `User`), their interfaces live in `Domain/Port`, web controllers are grouped by route, and `BaseController` moved to `Application\Controller`. Behaviour is unchanged; addon code that imports these classes needs its `use` lines updated | [Development Setup](../contributing/development.md#project-structure) |
