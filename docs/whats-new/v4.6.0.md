...
| `[DOMAIN]` and `[TLD]` follow the public suffix list | Zone templates split a zone name where registration actually happens, so `example.com.pl` resolves `[DOMAIN]` to `example` and `[TLD]` to `com.pl`. Previously only about twenty suffixes such as `co.uk` were recognised and everything else split on the last dot | [DNS Templates](../user-guide/dns-templates.md) |
| SQLite runs in WAL mode | PowerDNS reads the same file Poweradmin writes, and the old journal let a reader block a zone save with `database is locked`. SQLite now opens in write-ahead logging, which adds `-wal` and `-shm` files next to the database and changes how it must be backed up | [SQLite Configuration](../database/sqlite.md#write-ahead-logging) |
...
| A zone always keeps an owner | Removing the last owner is refused wherever it could happen: removing an owner or a group, the quick-remove control, removing several zones from a group at once, and deleting a group that is the last owner. The message names the zone, and the v2 API answers 409 with the same text | [Permissions](../user-guide/permissions.md#zones-always-keep-an-owner) |
| Record counts and comments on the full zone list | Asking for every zone at once reported an inflated record count on zones with comments and dropped the zone comment; both now match the paginated list | |
...
| `bin/poweradmin` console | A first command-line entry point with `zone:list` and `zone:show`, running the same use cases as the web interface without a session (`bin/poweradmin --as-user=1 zone:show example.com`). Output is tab separated or JSON with `--format=json` | [Development Setup](../contributing/development.md#project-structure) |
