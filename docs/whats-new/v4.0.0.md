...
on) instead of a flat list of constants. A `config/migrate-config.php` helper converted an
existing 3.x file; both it and support for the old format were removed in 4.1.0, so upgrades
from v3.x are now a manual rewrite.
...
| Database consistency checks | An admin page that scans for orphaned and inconsistent DNS data | [Database Consistency Check](../maintenance/consistency-check.md) |
| PowerDNS status page | Server health and statistics from inside Poweradmin | [PowerDNS API](../configuration/powerdns-api.md) |
| Email template previews | Preview transactional emails in the browser, light and dark | [Mail Configuration](../configuration/mail.md) |
...
| Release | Added |
|---------|-------|
| v4.0.1 | `PA_PDNS_DB_NAME` for pointing Docker at a separate PowerDNS database (the `database.pdns_db_name` setting itself dates to 3.8.0) |
