...
- Zone lists in API backend mode are considerably faster on installations with many zones: record counts are now fetched only for the zones on the current page instead of for every zone, and repeated reads of the same PowerDNS data are reused within a request. As a consequence the Records column is no longer sortable in API backend mode - counts are known only for the visible page, so ordering by them would sort just that page. It remains sortable in SQL backend mode.
