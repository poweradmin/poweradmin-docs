...
**Separate PowerDNS database (MySQL/MariaDB with `pdns_db_name`)**

The 4.3.0 MySQL upgrade script backfills `zones.zone_name`, `zone_type`, and `zone_master` from the PowerDNS `domains` table. If you have `pdns_db_name` set (PowerDNS tables live in a separate database), open `sql/poweradmin-mysql-update-to-4.3.0.sql` and set `@pdns_db` to that database name before running it:

```sql
SET @pdns_db = 'pdns';
```

Your MySQL user needs `SELECT` on that database. If the `domains` table is not found in `@pdns_db`, the script prints `PowerDNS domains table not found, zone backfill skipped` and still applies everything else; the backfill only matters if you later switch the installation to the API backend.
...
**If you ran the script shipped with 4.3.0 to 4.3.5 with a separate PowerDNS database**, it stopped at `Table 'poweradmin.domains' doesn't exist` and everything after that statement was skipped: the `users.perm_templ_source` column, the `zones.zone_name` unique index, the wider `record_comment_links.record_id`, and the `log_api` table. The visible symptom is `An error occurred while processing the request.` on every user save. Run the rest of the script by hand against your Poweradmin database, replacing `pdns` with your `pdns_db_name`:
...
Quote the database and table names separately: `` `pdns`.`domains` ``, not `` `pdns.domains` ``.
