...
Quote the database and table names separately. Writing `` `pdns.domains` `` - a single pair of backticks around both names - makes MySQL look for one table literally named `pdns.domains` inside your Poweradmin database, which fails with `Table 'poweradmin.pdns.domains' doesn't exist`.
