...
See the [OIDC Configuration](../configuration/oidc.md) and [SAML Configuration](../configuration/saml.md) guides for setup instructions.
...
Access wizards from the zone edit page or the DNS Wizards menu. See [DNS Wizards](../configuration/dns-wizards.md) for details.
...
Requires email configuration. See [Mail Configuration](../configuration/mail.md).
...
- The `try_files` directive routes requests to `index.php` — use the [nginx.conf.example from master](https://github.com/poweradmin/poweradmin/blob/master/nginx.conf.example)

**Caddy users must ensure:**

- The `try_files` directive is configured — use the [caddy.conf.example from master](https://github.com/poweradmin/poweradmin/blob/master/caddy.conf.example)

!!! warning "Symptom: 404 on /login after upgrade"
    If you see a 404 error when Poweradmin redirects to `/login`, your web server is not rewriting URLs to `index.php`. This is the most common issue when upgrading from v4.0.x to v4.1.0. See the troubleshooting section below.
...
2. **Apache**: Verify `AllowOverride All` is set in your VirtualHost:
```apache
...
3. **Apache**: Verify the `.htaccess` file is present in the Poweradmin root directory and is the version from v4.1.0 (not v4.0.x)

4. **Nginx**: Verify your configuration includes the `try_files` directive:
```nginx
location / {
...
5. **All servers**: Check error logs for more details:
```bash
# Apache
...
Users upgrading from v4.0.x may encounter a 404 error when Poweradmin redirects to `/login`. This happens because v4.1.0 uses clean URLs that require URL rewriting to be properly configured on the web server. See the [Breaking Changes](#url-rewriting-now-required) section above for the fix.

If you encounter any other issues, please report them on [GitHub Issues](https://github.com/poweradmin/poweradmin/issues).
...
- [DNS Wizards](../configuration/dns-wizards.md)
- [Custom CSS Theming](../configuration/ui/custom-css.md)
...
For a detailed history of all changes, see the [GitHub commit history](https://github.com/poweradmin/poweradmin/commits/release/4.0.x).
