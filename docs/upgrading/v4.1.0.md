...
### Critical Security Fix: MFA Bypass (CVE #813)
...
**None** - Version 4.1.0 is fully backward compatible with v4.0.0 configuration and database schema.
...
**Important:** Version 4.1.0 has no database schema changes. If you're upgrading from v4.0.0-4.0.3, you may need to apply the v4.0.2 migration if not already applied.

**Check if you need the 4.0.2 migration:**

```sql
-- MySQL/MariaDB
SHOW CREATE TABLE password_reset_tokens;
-- Check if 'used' field has DEFAULT 0

-- PostgreSQL
\d password_reset_tokens
-- Check if 'used' field has default value
...
**If the `used` field doesn't have a default value, apply the 4.0.2 migration:**
...
### Database & Zone Management (v4.0.1 - v4.0.4)
...
**Note:** Since v4.1.0 has no database schema changes from v4.0.0, you can safely rollback to v4.0.0 without database migration reversals. If you applied the 4.0.2 migration, you can keep it applied as it's a minor fix.
...
Upgrading from v4.0.4 is straightforward - simply replace the files. No database changes are needed.
