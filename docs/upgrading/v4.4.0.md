...
Username recovery continues to send emails when `application_url` is empty, falling back to the web server's `SERVER_NAME`; if neither is available, the email omits the login link but still includes the username. See [Security Policies](../configuration/security-policies.md#interfaceapplication_url-is-required) and [Username Recovery](../configuration/username-recovery.md#requirements) for the full details.
